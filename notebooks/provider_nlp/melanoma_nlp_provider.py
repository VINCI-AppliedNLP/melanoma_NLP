...
preprocess_rules = [  # replace carriage return with newline for ease of viewing and consistency
    ...
    PreprocessingRule(  # Documents with many newline characters stall nlp indefinitely. This reduces these to spaces (preserving offsets), which are better handled
        "\n" * 10000,
        repl="\n" + " " * 9999,
    ),
    PreprocessingRule(
...
def table_import(clean_query: str, db_server: str, db_db: str) -> pd.DataFrame:
    """
    Imports a table from a SQL Server database into a pandas DataFrame.

    Parameters:
        clean_query (str): SQL-formatted string query to select the target table or data.
        db_server (str): The name or address of the SQL Server instance.
        db_db (str): The name of the database to connect to.

    Returns:
        pd.DataFrame: DataFrame containing the results of the SQL query.
        None: If an error occurs during the database connection or query execution.

    Raises:
        Prints an error message if the connection or query fails.
    """
    db_conn_str = (
...
def csv_to_targetmatcher(filepath=None, insert_whitespace_matching=False) -> List[medspacy.ner.TargetRule]:
    ...
    Args:
        filepath (str, optional): Path to the CSV file containing rule definitions. The CSV should have columns 'pattern', 'label', and 'literal'.
        insert_whitespace_matching (bool, optional): If True, inserts regex patterns for whitespace between each token in the rule pattern to allow flexible matching.
    Returns:
        List[medspacy.ner.TargetRule]: A list of TargetRule objects constructed from the CSV file.
    Notes:
        - The 'pattern' column should contain either a string or a list of dictionaries representing spaCy token match patterns.
        - If 'pattern' cannot be parsed as a list, it will be treated as a string for matching.
        - Whitespace matching is inserted as additional token patterns when insert_whitespace_matching is True.
    """
    import pandas as pd
    from medspacy.ner import TargetRule
    import ast
    ...
    for i, row in df.iterrows():
        patt = None
        try:
            patt = ast.literal_eval(
                row["pattern"]
            )  # ast.literal_eval(repr(row['pattern'])[1:-1])##ast.literal_eval(repr(row['pattern'])[1:-1])#fix backslashes
            ...
            if (str(row["pattern"]) != "nan") and (str(row["pattern"]) != "None"):
                patt = [
                    {"LOWER": {"REGEX": row["pattern"]}}
                ]  # str(repr(str(row['pattern']))[1:-1])#fix backslashes
...
def csv_to_contextrules(filepath=None, insert_whitespace_matching=False) -> List[medspacy.context.ConTextRule]:
    ...
    Args:
        filepath (str, optional): Path to the CSV file containing rule definitions. The CSV should have columns 'pattern', 'label', and 'literal'.
        insert_whitespace_matching (bool, optional): If True, inserts regex patterns for whitespace between each token in the rule pattern to allow more flexible matching.
    Returns:
        List[medspacy.context.ConTextRule]: A list of ConTextRule objects constructed from the CSV file.
    Notes:
        - The 'pattern' column should contain either a string or a list of dictionaries representing spaCy token match patterns.
        - If 'pattern' cannot be parsed as a list, it will be treated as a string for matching.
        - Whitespace matching is inserted as additional token patterns when insert_whitespace_matching is True.
...
def targetrules_to_df(
    targetrules,
    remove_whitespace_matching=True,
    include_direction=False,
    include_scope=False,
) -> pd.DataFrame:
    """
    Converts a list of target rule objects into a pandas DataFrame for easier storage and viewing.
    Parameters
    ----------
    targetrules : list
        A list of target rule objects, each expected to have attributes: 'literal', 'category', 'pattern', and optionally 'direction' and 'max_scope' (For ContextRules)
    remove_whitespace_matching : bool, optional (default=True)
        If True, removes whitespace matching patterns by selecting every fourth element from the pattern list.
    include_direction : bool, optional (default=False)
        If True, includes the 'direction' attribute from each rule in the resulting DataFrame.
    include_scope : bool, optional (default=False)
        If True, includes the 'max_scope' attribute from each rule in the resulting DataFrame.
    Returns
    -------
    pd.DataFrame
        A DataFrame containing the extracted rule information with columns: 'literal', 'label', 'pattern', and optionally 'direction' and 'scope'.
    """
...
def check_terminators(target, modifier, span_between):
    ...
    """
    terminators = ["MARGINS"]  # May want to eliminate Conj from this
...
def check_terminators_comment(target, modifier, span_between):
    ...
    for i in range(len(span_between_tags) - 1):
        if span_between_tags[i] == "NEWLINE" and span_between_tags[i + 1] == "NEWLINE":
            return False
    if "[x]" in span_between.text:
        return False
    return True


def check_terminators_comment_extra_newline(target, modifier, span_between):
    """
    Excludes cases where certain characters are between modifiers and the target.
    """
    span_between_tags = [tok._.concept_tag for tok in span_between]
    for i in range(len(span_between_tags) - 2):
        if (
            span_between_tags[i] == "NEWLINE"
            and span_between_tags[i + 1] == "NEWLINE"
            and span_between_tags[i + 2] == "NEWLINE"
        ):
            return False
    if "[x]" in span_between.text:
        return False
...
def check_met_scope(target, modifier, span_between):
    if (modifier.start < target.start) & (len(span_between) > 10):
        return False
    return True
...
def add_onmatch_terminators(rule_list):
    ...
    for rule in rule_list:
        if rule.literal in ["comment", "note:", "generic staging info"]:
            context_rules_upd.append(
                ConTextRule(
                    ...
                    terminated_by=["NONSKIN_TOPOGRAPHY", "SKIN_TOPOGRAPHY"],
                    on_modifies=check_terminators_comment_extra_newline,
                    allowed_types=["MEL_UNSPEC", "METASTASIS"],
                )
            )
        elif rule.literal in ["[]", "[ ]", ":0"]:
            context_rules_upd.append(
                ConTextRule(
                    literal=rule.literal,
                    category=rule.category,
                    ...
                    max_targets=rule.max_targets,
                    terminated_by=["NONSKIN_TOPOGRAPHY", "SKIN_TOPOGRAPHY"],
                    on_modifies=check_terminators_comment,
                )
            )
        elif rule.category == "HISTOLOGY_TYPE":
            context_rules_upd.append(
                ConTextRule(
                    literal=rule.literal,
                    category=rule.category,
                    ...
                    max_targets=rule.max_targets,
                    terminated_by=["NONSKIN_TOPOGRAPHY", "SKIN_TOPOGRAPHY"],
                    allowed_types=["HISTOLOGY", "MEL_UNSPEC"],
                )
            )
        elif rule.category in ["METASTATIC", "NEGATED_METASTATIC"]:
            # print("""adjusting met scope twice""")
            # print(ConTextRule(literal=rule.literal+"_BACKWARD", category=rule.category, pattern=rule.pattern, direction="BACKWARD",max_scope=12,max_targets=1,terminated_by=["NOT_MELANOMA_DX","TUMOR_HIST"]))
            # print(ConTextRule(literal=rule.literal, category=rule.category, pattern=rule.pattern, direction="FORWARD",max_scope=120,max_targets=1,terminated_by=["NOT_MELANOMA_DX","TUMOR_HIST"]))
            # context_rules_upd.append(ConTextRule(literal=rule.literal+"_BACKWARD", category=rule.category+"_BACKWARD", pattern=rule.pattern, direction="BACKWARD",max_scope=120,max_targets=1,terminated_by=["NOT_MELANOMA_DX","TUMOR_HIST"])) # ,terminated_by=['NONSKIN_TOPOGRAPHY','SKIN_TOPOGRAPHY'],allowed_types=["HISTOLOGY","MEL_UNSPEC"]
            context_rules_upd.append(
                ConTextRule(
                    literal=rule.literal,
                    category=rule.category,
                    ...
                    on_modifies=check_met_scope,
                    terminated_by=["NONSKIN_TOPOGRAPHY", "SKIN_TOPOGRAPHY"],
                )
            )  # terminated_by=["NOT_MELANOMA_DX","TUMOR_HIST"]
            ...
            ("DEEP_TRANSECTION" in rule.category)
            or ("NEGATED_TRANSECTION" in rule.category)
            or ("UNSPEC_TRANSECTION" in rule.category)
        ):
            ...
            context_rules_upd.append(
                ConTextRule(
                    ...
                    max_targets=rule.max_targets,
                    terminated_by=["NONSKIN_TOPOGRAPHY", "SKIN_TOPOGRAPHY"],
                    allowed_types=["HISTOLOGY", "MEL_UNSPEC", "NOT_MELANOMA_DX"],
                )
            )
        elif rule.literal in [
            "Topography_group5_firsttok",
            "Topography_group8_firsttok",
        ]:
            context_rules_upd.append(
                ConTextRule(
                    literal=rule.literal,
                    ...
                    max_targets=rule.max_targets,
                    terminated_by=["NONSKIN_TOPOGRAPHY", "SKIN_TOPOGRAPHY"],
                    on_modifies=first_token_callback,
                )
            )
...
"""    
    ...
    The pipeline includes:
        ...
        - Pretagger for initial entity marking based on imported rules.
        - Concept matcher for identifying key clinical concepts.
        - List tagger for detecting comma-separated lists.
        - Target matcher for extracting target entities.
        - Context processing for negation, histology, ulceration, metastasis, and other clinical modifiers.
        - Sequential context layers for improved modifier detection.
        - Custom entity creation for melanoma abbreviations, metastasis, and histology types.
    Rule files (JSON format) for taggers and context are loaded from the specified `file_path`.
    Args:
        file_path (str): Directory path containing rule files (default: "./").
        csv_or_json_rule_import (str): Format for rule import, either "json" or "csv" ("csv" deprecated; default: "json").
        insert_whitespace_matching (bool): Whether to insert whitespace matching in tokenization (default: False).
    Returns:
...
"""
    ...
    try:
        pretagger_rules = TargetRule.from_json(
            os.path.join(file_path, "pretagger_rules.json")
        )
        tagger_rules = TargetRule.from_json(
            os.path.join(file_path, "tagger_rules.json")
        )
        target_rules = TargetRule.from_json(
            os.path.join(file_path, "target_rules.json")
        )
        context_rules = []
        context_json_list = [
            "depth_context_rules.json",
            "transect_context_rules.json",
            "mitotic_index_context_rules.json",
            "metastasis_context_rules.json",
            "top_hist_context_rules.json",
            "ulceration_context_rules.json",
            "template_exclusion_context_rules.json",
            "unused_terminate_context_rules.json",
        ]
        ...
        context_rules_mod = add_onmatch_terminators(
            ConTextRule.from_json(
                os.path.join(file_path, "modifier_context_rules.json")
            )
            + ConTextRule.from_json(
                os.path.join(file_path, "top_hist_context_rules.json")
            )
        )
    except Exception as e:
    ...
    context = nlp.add_pipe(
        "medspacy_context",
        config={
            "rules": None,
            "max_scope": CONTEXT_SCOPE,
            "allowed_types": ALLOWED_TYPES,
            "prune_on_target_overlap": False,
        },
    )
    ...
    @Language.component("make_mel_abbrev_ent")
    def make_mel_abbrev_ent(doc):
        for ent in doc.ents:
            for mod in ent._.modifiers:
                if mod.category == "HISTOLOGY_ABBREV":
                    start, end = mod.modifier_span
                    new_ent = Span(doc, start, end, label="MEL_UNSPEC")
                    # new_ent._.modifiers = ent._.modifiers #these are considered coreferences, acquiring the same context
                    try:
                        doc.ents += (new_ent,)
                    except:
                        pass
    ...
    nlp.add_pipe("make_mel_abbrev_ent", name="make_mel_abbrev_ent", last=True)

    @Language.component("make_met_ent")
    def make_met_ent(doc):
        for ent in doc.ents:
            if ent.label_ in ["HISTOLOGY", "MEL_UNSPEC"]:
                for mod in ent._.modifiers:
                    if mod.category in ["METASTATIC", "METASTATIC_BACKWARD"]:
                        start, end = mod.modifier_span
                        new_ent = Span(doc, start, end, label="METASTASIS")
                        try:
                            doc.ents += (new_ent,)
                        except:
                            pass
    ...
    nlp.add_pipe("make_met_ent", name="make_met_ent", last=True)

    # Second context for modifiers (sequentially after initial context, so that metastasis and mel abbrev have modifiers
    ...
    context_mod.add(context_rules_mod)

    @Language.component("medspacy_context_custom")
    def medspacy_context_custom(doc):
        return context_mod(doc)
...
def map_histology(ent):
    ...
    if ent.label_ in [
        "HISTOLOGY",
        "HISTOLOGY_TYPE",
    ]:  # Labels that have specific histologies
        hist_ext = list(
            set(
                [
                    tok._.pretag.replace(" ", "_").replace("'", "")
                    for tok in ent
                    if tok._.concept_tag in ["HISTOLOGY_TERM"]
                ]
            )
        )
...
def measurement_extract(span):
    ...
    for tok in span:
        if tok._.concept_tag == "VALUE":
            ...
            ext_units = re.findall(
                r"cm|mm|millimeters?|centimeters?", tok.text, re.IGNORECASE
            )
    ...
    if (len(units) == 1) and (len(values) == 1):
        ...
        and (len(values) == 2)
        and (
            (round(values[0], 0) == round(round(values[1] * 10, 0)))
            or (round(values[1], 0) == round(values[0] * 10, 0))
        )
    ):  # cases where both the cm and mm are reported (one value must be 10x the other)
        return max(values)  # take the larger number as the mm
    elif (len(values) > 1) and (
        len(units) > 0
    ):  # If multiple values, arbitrarily choose first value (should typically be the case)
        if units[0] == "mm":
            return values[0]
        if units[0] == "cm":
            return values[0] * 10
...
def MI_extract_full(span):
    ...
    num1, num2, qualitative, hpf_num1, hpf_num2, hpf_den = [
        None,
        None,
        None,
        None,
...
def map_mod(mod, span, ent_label, doc):
    """
    Maps a modifier object, text span, and entity label to a standardized label and value for melanoma provider NLP extraction.
    Args:
        mod: An object representing a modifier, expected to have a 'category' attribute indicating the type of modifier.
        span: An object representing a text span, expected to have a 'text' attribute containing the relevant text.
        ent_label: A string representing the entity label associated with the modifier.
    Returns:
        tuple: A tuple (lab, val) where:
            lab (str or None): The standardized label corresponding to the modifier category, or "IGNORE" if not applicable.
            val (various or None): The extracted value associated with the label, which may be a string, integer, or None depending on the category.
    """
    ...
    if mod.category in ["DEPTH", "MELANOMA_DEPTH"]:
        ...
        try:
            ...
            # value = -1
            value = "EXT_ERROR"
    elif mod.category in [
        "DEPTH_NO_VALUE",
        "MELANOMA_DEPTH_NO_VALUE",
        "T_STAGING_GUIDELINE",
        "UNCERTAIN_NO_MEASUREMENT",
        "UNCERTAIN_MEASUREMENT",
        "NOT_APPLICABLE",
        "NOT_MEASURED",
        "SUPERFICIAL_BIOPSY",
        "SUPERFICIALLY_INVASIVE",
        "NEGATED_METASTATIC",
        "PERIPHERAL_TRANSECTION",
    ]:  # ,'ULCERATION_TERM']: #Unsure about uncertain/not-applicable/non-measured breslow
        lab = "IGNORE"
        val = None
        ...
        "ULCERATION_TERM"
    ]:  ###### Need to make sure context is set up for this then extract negation
        if span.text.strip() != "":
            lab = "ulceration_status"
            val = "present"
        else:
            lab = "IGNORE"
            val = None
    # elif mod.category in ['METASTATIC']: #05-12 change
    #    print(ent_label)
    #    if ent_label in ["HISTOLOGY","HISTOLOGY_TYPE","MEL_UNSPEC"]:
    #        for ent in doc.ents: #Find the metastasis entity object for the metastasis modifier
    #            if ent.start == mod.modifier_span[0]:
    #                if (ent._.is_uncertain) | (ent._.is_family) | (ent._.is_negated) | (ent._.is_hypothetical):
    #                    lab = 'metastasis'
    #                    val = 0
    #                else:
    #                    lab = 'metastasis'
...
def transect_depth_ext(span):
    """Helper function to determine if depth is greater than breslow measurement"""
...
columns_top_grouped = [
    "doc_id",
    "Topography",
    "Topography_start_span",
    "Topography_end_span",
    "skin_topography_present",
    "breslow_depth_mm",
    "clark_level",
    "metastasis",
    "metastasis_historical",
    "metastasis_negated",
    "metastasis_hypothetical",
    "metastasis_is_possible_existence",
    "nonskin_melanoma_dx",
    "deep_margin_involvement",
    "unspecific_or_uncertain_margin_involvement",
    "deep_margin_involvement_negated",
    "non_melanoma_transection",
    "depth_greater_than_breslow_measurement",
    "ulceration_status",
    "Mitotic_index",
    "pending",
    "cutaneous",
    ...
    "spindle_cell",
    "melanoma_not_otherwise_specified",
    "nevoid",
    "ocular",
    "desmoplastic",
    "melanoma_negated",
    "melanoma_hypothetical",
    "melanoma_is_possible_existence",
    "melanoma_historical",
    "cancer_negated",
    "non_melanoma_dx",
    "cancer_unspecified",
    "non_melanoma_dx_negated",
    "cancer_unspecified_negated",
    "non_melanoma_dx_historical",
    "cancer_unspecified_historical",
    "non_melanoma_dx_hypothetical",
    "cancer_unspecified_hypothetical",
    "non_melanoma_dx_is_possible_existence",
    "cancer_unspecified_is_possible_existence",
]
...
def data_transformation(docIDs, docs):
    """
    Transforms and encodes pathology NLP data for melanoma analysis.
    This function processes a list of document IDs and their corresponding documents,
    extracting relevant information. The transformation includes flattening relationships,
    one-hot encoding histological features, and grouping top-level data. If no data is found,
    returns empty or None values for each output.
    Parameters
    ----------
    docIDs : list
        List of document IDs to process.
    docs : list
        List of document texts corresponding to the docIDs.
    Returns
    -------
    dict
        A dictionary containing the following keys:
            - "long_df": DataFrame with extracted entities and relationships (or None).
            - "piv_1": DataFrame flattened on relationships (or None).
            - "piv_1_encoded": One-hot encoded DataFrame of histological features (or None).
            - "top_grouped": Grouped DataFrame with top-level features for each document.
    """
...
def spacy_to_df_melanoma(docIDs, docs):
    ...
    Args:
        docIDs (list): List of document identifiers corresponding to each spaCy document.
        docs (list): List of spaCy Doc objects containing named entities and custom extensions.
    Returns:
        pandas.DataFrame: A DataFrame in long format where each row represents an entity or modifier 
        extracted from the documents, including metadata such as document ID, entity text, label, 
        character offsets, mapped histology, modifier details, and extended labels/values.
    """
    long_data = []
        ...
        for ent in doc.ents:
            ...
            if len(ent._.modifiers) > 0:
                for mod in ent._.modifiers:
                    ...
                    mod_ext_label, mod_ext_value = map_mod(mod, span, ent.label_, doc)
                    try:
                        mod_end_char = doc[
                            mod.modifier_span[1]
                        ].idx  # throws error if index is at the end of doc
                    except:
                    ...
                    if mod.category in ["DEPTH", "MELANOMA_DEPTH", "CLARK_DEPTH"]:
                        try:
                            span = doc[
                                mod.modifier_span[0] - 12 : mod.modifier_span[1] + 4
                            ]
                        except:
                            span = doc[mod.modifier_span[0] : mod.modifier_span[1]]
                        mod_ext_label, mod_ext_value = (
                            "depth_greater_than_breslow_measurement",
                            transect_depth_ext(span),
                        )
                        long_data.append(
                            {
                                "doc_id": docID,
    ...
    long_df.loc[
        (long_df["anchor_mapping"] == "in_situ")
        & (
            long_df["modifier_ext_label"]
            == "unspecific_or_uncertain_margin_involvement"
        ),
        "modifier_ext_label",
    ] = "PERIPHERAL_TRANSECTION"
...
def flatten_on_relationships(long_df):
    """
    Converts a long-format DataFrame of NLP relationships into a wide-format DataFrame.
    Pivots on anchor and modifier columns, aggregates modifier values, and extracts context columns.
    Adds character span info for topography modifiers.
    Args:
        long_df (pd.DataFrame): Input long-format DataFrame.
    Returns:
        pd.DataFrame: Wide-format DataFrame with extracted modifier/context values.
    """
    long_df["modifier_ext_value"] = long_df.apply(
        lambda row: (
            f"{row['modifier_ext_value']}_{row['modifier_start_char']}_{row['modifier_end_char']}"
            if row["modifier_ext_label"] in ["Skin_Topography", "Nonskin_Topography"]
            else row["modifier_ext_value"]
        ),
        axis=1,
    )
    piv_1 = long_df.pivot_table(
        index=[
            "doc_id",
            "anchor_text",
            "anchor_mapping",
        ...
        columns="modifier_ext_label",
        values="modifier_ext_value",
        aggfunc=list,
    )
    ...
    for col in [
        ...
        "is_historical",
        "is_hypothetical",
        "is_possible_existence",
        "is_family",
    ]:  # ,'metastasis'#05-12 change
    ...
    piv_1.is_family = piv_1.is_family.apply(first_return)
...
def one_hot_encode_hist(piv_1, exclude_nonmelanoma_ulceration=1):
    """
    One-hot encodes 'anchor_mapping' and processes metastasis/topography features with respect to negation, historical, hypothetical, and possible existence flags.
    Ensures expected columns exist and optionally sets 'ulceration_status' to None for non-melanoma diagnoses.
    Args:
        piv_1 (pd.DataFrame): Input DataFrame.
        exclude_nonmelanoma_ulceration (int, optional): If 1, sets 'ulceration_status' to None for non-melanoma diagnoses.
    Returns:
        pd.DataFrame: Processed and encoded DataFrame.
    """
    # if the cancer is negated or historical, capture this separately
    piv_1["is_negated"] = piv_1["is_negated"].fillna(0)
    piv_1["is_family"] = piv_1["is_family"].fillna(0)
    piv_1["anchor_mapping"] = piv_1.apply(
        lambda x: (
            f"{x['anchor_mapping']}_historical"
            if (
                (x["is_historical"] == 1)
                & (x["is_negated"] == 0)
                & (x["is_family"] == 0)
            )
            else x["anchor_mapping"]
        ),
        axis=1,
    )
    piv_1["anchor_mapping"] = piv_1.apply(
        lambda x: (
            f"{x['anchor_mapping']}_family"
            if ((x["is_family"] == 1) & (x["is_negated"] == 0))
            else x["anchor_mapping"]
        ),
        axis=1,
    )
    ...
    piv_1["anchor_mapping"] = piv_1.apply(
        lambda x: (
            f"{x['anchor_mapping']}_hypothetical"
            if (
                (x["is_hypothetical"] == 1)
                & (x["is_negated"] == 0)
                & (x["is_family"] == 0)
    ...
    piv_1["anchor_mapping"] = piv_1.apply(
        lambda x: (
            f"{x['anchor_mapping']}_is_possible_existence"
            if (
                (x["is_possible_existence"] == 1)
                & (x["is_negated"] == 0)
                & (x["is_family"] == 0)
            )
            else x["anchor_mapping"]
    ...
    # piv_1['metastasis_historical'] = piv_1.apply(lambda x: 1 if ((x['is_historical'] == 1) & (x['is_negated'] == 0) & (x['is_family'] == 0) & (x["metastasis"] == 1)) else 0, axis=1)
    # piv_1['metastasis_family'] = piv_1.apply(lambda x: 1 if ((x['is_historical'] == 1) & (x['is_negated'] == 0) & (x['is_family'] == 0) & (x["metastasis"] == 1)) else 0, axis=1)
    ...
    # piv_1['metastasis'] = piv_1.apply(lambda x: 0 if ((x["metastasis_historical"] == 1) | (x["metastasis_negated"] == 1) | (x["metastasis_hypothetical"] == 1) | (x["metastasis_is_possible_existence"] == 1)) | (x['metastasis_family'] == 1) | (x["metastasis_family"] == 1) else x["metastasis"], axis=1) #metastasis should be 0 if it's historical or negated or hypothetical (this is captured separately)

    one_hot_encoded_hist = pd.get_dummies(piv_1["anchor_mapping"])
    piv_1_encoded = pd.concat([piv_1, one_hot_encoded_hist], axis=1)
    piv_1_encoded["skin_topography_present"] = piv_1_encoded.Skin_Topography.apply(
        lambda x: 1 if pd.notna(x) else 0
    )
    piv_1_encoded["Topography"] = piv_1_encoded.apply(
        lambda row: (
            row["Skin_Topography"]
            if pd.notna(row["Skin_Topography"])
            else row["Nonskin_Topography"]
        ),
        axis=1,
    )
    piv_1_encoded["Topography_start_span"] = piv_1_encoded.Topography.apply(
        lambda x: x.split("_")[-2] if pd.notna(x) else None
    )
    piv_1_encoded["Topography_end_span"] = piv_1_encoded.Topography.apply(
        lambda x: x.split("_")[-1] if pd.notna(x) else None
    )
    piv_1_encoded["Topography"] = piv_1_encoded.Topography.apply(
        lambda x: "".join(x.split("_")[:-2]) if pd.notna(x) else None
    )
    ...
    for col in (
        ...
        + melanoma_dx
        + other_dx
        + [f"{x}_negated" for x in melanoma_dx]
        + [f"{x}_historical" for x in melanoma_dx]
        + [f"{x}_family" for x in melanoma_dx]
        + [f"{x}_negated" for x in other_dx]
        + [f"{x}_historical" for x in other_dx]
        + [f"{x}_hypothetical" for x in melanoma_dx]
        + [f"{x}_family" for x in other_dx]
        + [f"{x}_hypothetical" for x in other_dx]
        + [f"{x}_is_possible_existence" for x in melanoma_dx]
        + [f"{x}_is_possible_existence" for x in other_dx]
        + [
            "metastasis_negated",
            "metastasis_historical",
            "metastasis_family",
    ...
    if exclude_nonmelanoma_ulceration:
        piv_1_encoded.loc[
            piv_1_encoded["anchor_mapping"].isin(["non_melanoma_dx"]),
            "ulceration_status",
        ] = None
...
melanoma_cutaneous = [
    ...
    "hutchinsons_melanotic_freckle",
    "balloon_cell",
]  #'melanoma_unspecified' must also have no in-situ or 'lentigo_maligna' in order to count
...
def grouping_top(piv_1_encoded, return_max_values=1):
    """
    Groups and aggregates NLP data by topography and related spans, applying custom aggregation logic for melanoma features.
    Args:
        piv_1_encoded (pd.DataFrame): Input DataFrame containing encoded pathology data.
        return_max_values (bool, optional): If True, returns maximum values for numeric aggregations; otherwise returns all unique values. Default is True.
    Returns:
        pd.DataFrame: Aggregated DataFrame with melanoma and pathology features grouped by document and topography.
    """
    piv_1_encoded[["Topography", "Topography_start_span", "Topography_end_span"]] = (
        piv_1_encoded[
            ["Topography", "Topography_start_span", "Topography_end_span"]
        ].fillna("None")
    )
    top_grouped = piv_1_encoded.groupby(
        ["doc_id", "Topography", "Topography_start_span", "Topography_end_span"]
    ).agg(list)
    ...
    def max_breslow(mixed_list):
        ...
        flat_list = [
            num for num in flat_list if num not in [None, "EXT_ERROR"] and pd.notna(num)
        ]
        if len(flat_list) > 0:
        ...
        flat_list = list(
            set(
                [
                    str(el)
                    for el in flat_list
                    if el not in [None] and isinstance(el, str)
                ]
            )
        )
        if len(flat_list) > 0:
    ...
    def max_Mitotic_index(mixed_list):
        ...
        flat_list = [
            num for num in flat_list if num not in [None, "NA"] and pd.notna(num)
        ]
        flat_list_num = []
        for el in flat_list:
            if el != None:
    ...
    top_grouped["ulceration_status"] = top_grouped["ulceration_status"].apply(
        ulceration_priority
    )
    top_grouped["clark_level"] = top_grouped["clark_level"].apply(max_breslow)
    top_grouped["melanoma_negated"] = (
        top_grouped[[f"{x}_negated" for x in melanoma_dx_noinsitu]]
        .any(axis=1)
        .astype(int)
    )
    top_grouped["melanoma_hypothetical"] = (
        top_grouped[[f"{x}_hypothetical" for x in melanoma_dx_noinsitu]]
        .any(axis=1)
        .astype(int)
    )
    top_grouped["melanoma_is_possible_existence"] = (
        top_grouped[[f"{x}_is_possible_existence" for x in melanoma_dx_noinsitu]]
        .any(axis=1)
        .astype(int)
    )
    top_grouped["melanoma_historical"] = (
        top_grouped[[f"{x}_historical" for x in melanoma_dx_noinsitu]]
        .any(axis=1)
        .astype(int)
    )
    top_grouped["melanoma_family"] = (
        top_grouped[[f"{x}_family" for x in melanoma_dx_noinsitu]]
        .any(axis=1)
        .astype(int)
    )

    top_grouped.loc[
        (top_grouped["in_situ_historical"] != 1)
        & (top_grouped["lentigo_maligna_historical"] != 1)
        & (top_grouped["melanoma_unspecified_historical"] == 1),
        "melanoma_historical",
    ] = 1

    top_grouped["Mitotic_index"] = top_grouped["Mitotic_index"].apply(max_Mitotic_index)
    top_grouped.loc[
        top_grouped.Topography.str.contains("eye", case=False)
        & (~top_grouped.Topography.str.contains("eyelid", case=False))
        & (top_grouped.melanoma_unspecified == 1),
        "ocular",
    ] = 1  # eye topographies are ocular melanoma
    top_grouped.loc[
        top_grouped.Topography.str.contains("sinus", case=False)
        & (top_grouped.melanoma_unspecified == 1),
        "mucosal",
    ] = 1  # sinus topographies are mucosal melanoma
    ...
    top_grouped.loc[
        (top_grouped["in_situ"] != 1)
        & (top_grouped["lentigo_maligna"] != 1)
        & (top_grouped["mucosal"] != 1)
        & (top_grouped["ocular"] != 1)
        & (top_grouped["melanoma_unspecified"] == 1),
        "cutaneous",
    ] = 1
    ...
    if return_max_values:
        top_grouped.loc[
            (top_grouped["breslow_depth_mm"] > 0)
            & (
                (top_grouped["melanoma_unspecified"] == 1)
                | (top_grouped["lentigo_maligna"] == 1)
            ),
        ...
        top_grouped.loc[
            (
                top_grouped["breslow_depth_mm"].apply(
                    lambda x: isinstance(x, (set, list)) and (any(v > 0 for v in x))
                )
            )
            & (
                (top_grouped["melanoma_unspecified"] == 1)
    ...
    top_grouped.loc[
        (top_grouped["melanoma_unspecified"] == 1)
        & (top_grouped["Topography"].notna() & (top_grouped["Topography"] != "None"))
        & (top_grouped["skin_topography_present"] == 0),
        "nonskin_melanoma_dx",
    ] = 1
    top_grouped["cancer_negated"] = (
        top_grouped[["melanoma_negated", "cancer_unspecified_negated"]]
        .any(axis=1)
        .astype(int)
    )
    ...
    return top_grouped[
        ...
        + [f"{x}_negated" for x in other_dx]
        + [f"{x}_historical" for x in other_dx]
        + ["in_situ_historical", "melanoma_unspecified_historical"]
        + [f"{x}_family" for x in other_dx]
        + [f"{x}_hypothetical" for x in other_dx]
        + [f"{x}_is_possible_existence" for x in other_dx]
    ]
...
def doc_level_mel(top_grouped):
    """
    Aggregates melanoma-related features at the document level by taking the maximum value for each feature across
    all relevant topography spans.
    """
    return (
        top_grouped.groupby("doc_id")
        .agg(
            {
                "cutaneous": "max",
                "ocular": "max",
                "in_situ": "max",
                "mucosal": "max",
                "nonskin_melanoma_dx": "max",
                "metastasis": "max",
                "metastasis_negated": "max",
                "cancer_negated": "max",
            }
        )
        .reset_index()
...
def upload_to_cdw(
    ...
    try:
        ...
        res = [
            row.tolist()
            for row in df.replace("NA", None, inplace=False).fillna("None").values
        ]
        print(
            "\nResultant list of lists created for dataframe. First row of df: "
            + str(res[0])
        )
...
def transform_annot_df(annot_df, include_no_top_annot=True, binary_classification=True):
    ...
    Args:
        annot_df (pd.DataFrame): Annotation data.
        include_no_top_annot (bool): Include placeholder annotations for all documents if True.
        binary_classification (bool): Use binary values if True, else retain continuous/categorical.
    Returns:
        pd.DataFrame: Transformed annotation data per document (textID).
    """
    annot_df_adj = annot_df[
        annot_df["spanLabel"].isin(
            [
                "1_Cancer_Diagnosis",
                "2_Melanoma_Histology_Dx",
                "3_Histology_all_other_cancers",
                "Melanoma_Breslow_thickness",
                "Melanoma_Clark_level_of_invasion",
                "Melanoma_Ulceration",
                "Mitotic_index",
                "Metastasis",
                "Margins",
    ...
    annot_df_adj = annot_df_adj[
        annot_df_adj["Melanoma_Breslow_thickness_Presence"]
        != "Unable to determine or missing"
    ]
    annot_df_adj = annot_df_adj[
        ~(
            (annot_df_adj["spanLabel"] == "Metastasis")
            & (annot_df_adj["Metastasis_Site_Num1_of_metastasis"].isin(["None"]))
        )
    ]
    ...
    if include_no_top_annot:
        ...
        for i, textid in enumerate(annot_df_adj.textID.unique()):
            for col in annot_df_adj.columns:
                if col in [
                    ...
                    "Cancer_Diagnosis_Result",
                    "Cancer_Diagnosis_Temporality",
                ]:
        ...
        for textid in annot_df_adj.textID.unique():
            annot_df_adj.loc[
                (annot_df_adj["textID"] == textid)
                & (
                    (annot_df_adj["relID"] == "None")
                    | (
                        (
                            ~annot_df_adj["relID"].isin(
                                annot_df_adj.loc[
                                    annot_df_adj["textID"] == textid, "spanID"
                                ]
                            )
                        )
                        & (annot_df_adj["relID"] != "None")
                ...
                (annot_df_adj["textID"] == textid)
                & (annot_df_adj["spanLabel"] == "1_Cancer_Diagnosis")
            ][
                "spanID"
            ].tolist()[
                0
    ...
    annot_df_adj_breslow_transect = annot_df_adj[
        annot_df_adj["spanLabel"] == "Melanoma_Breslow_thickness"
    ].copy()
    ...
    for i, row in annot_df_adj.iterrows():
        ...
        if lab == "Melanoma_Breslow_thickness":
            mod_ext_lab.append(
                "breslow_measurement"
            )  # Already filtered out irrelevant cases in above cell
            if binary_classification:
                mod_ext_list.append(
                    1
                    if (not pd.isna(row["breslow_measurement"]))
                    | (row["Melanoma_Breslow_thickness_Presence"] == "Measurement")
                    else 0
                )
            else:
        ...
        if lab == "Mitotic_index":
            ...
            if binary_classification:
                if mitotic_ind == "Measurement":
                    ...
                    try:
                        mod_ext_list.append(
                            1 if (float(row["mitotic_measurement"]) > 0) else 0
                        )
                    except:
                        mod_ext_list.append(0)
            else:
        ...
        if lab == "1_Cancer_Diagnosis":
            ...
            if (
                (row["Cancer_Diagnosis_Status"] not in ["Negation"])
                and (
                    row["Cancer_Diagnosis_Result"]
                    not in ["Benign-NOT cancer", "Unable to determine or missing"]
                )
                and (row["Cancer_Diagnosis_Temporality"] != "Past")
            ):  # (row['Cancer_Diagnosis_Topography'] == 'Skin')
                mod_ext_list.append(1)
        ...
        if lab == "Breslow_transect":
            ...
            try:
                ...
                mod_ext_list.append(
                    1 if int(row["depth_greater_than_excision"]) == 1 else 0
                )
            except:
                mod_ext_list.append(0)
    ...
    annot_piv_1 = annot_df_adj.pivot_table(
        index=[
            "textID",
            "annotatedSpan",
            "spanStartChar",
    ...
    annot_piv_1_mod = annot_piv_1[annot_piv_1.spanLabel != "1_Cancer_Diagnosis"][
        [
            "relID",
            "Melanoma",
            "Non_Melanoma",
    ...
    annot_piv_1_dx = annot_piv_1[annot_piv_1.spanLabel == "1_Cancer_Diagnosis"][
        [
            "textID",
            "annotatedSpan",
            "spanStartChar",
    ...
    annot_dx_df = annot_piv_1_dx.merge(
        annot_piv_1_mod,
        left_on=["textID", "spanID"],
        right_on=["textID", "relID"],
        how="left",
    )
    annot_dx_df[
        ["Melanoma", "Non_Melanoma", "metastasis", "ulceration", "transection"]
    ] = (
        annot_dx_df[
            ["Melanoma", "Non_Melanoma", "metastasis", "ulceration", "transection"]
        ]
    ...
    annot_dx_df[["clark", "mitotic", "breslow_measurement"]] = annot_dx_df[
        ["clark", "mitotic", "breslow_measurement"]
    ].astype(float)

    ##Setting using max value per dx
    annot_dx_df = (
        annot_dx_df.groupby(["textID", "annotatedSpan", "spanStartChar", "spanEndChar"])
        .agg(max)
        .reset_index()
    )
    ...
    annot_dx_df.loc[annot_dx_df["breslow_measurement"] > 0, "Melanoma"] = (
        1  # Changed 3-21-2025
    )
    ...
    if binary_classification:
        ...
        annot_doc_df = (
            annot_doc_df.groupby("textID")
            .agg(
                {
                    "Melanoma": "max",
                    "breslow_measurement": lambda x: set(
                        [float(v) for v in x if pd.notna(v)]
                    ),
                    "clark": lambda x: set([v for v in x if pd.notna(v)]),
                    "metastasis": "max",
                    "ulceration": "max",
                    "mitotic": lambda x: set([float(v) for v in x if pd.notna(v)]),
                    "metastasis": "max",
                    "transection": "max",
                }
            )
    ...
    annot_doc_df[["melanoma", "ulceration_status", "Mitotic_index"]] = annot_doc_df[
        ["Melanoma", "ulceration", "mitotic"]
    ]
...
def transform_nlp_df(top_grouped, binary_classifier=True):
    ...
    nlp_df.loc[
        (
            nlp_df[["cutaneous", "mucosal", "ocular", "melanoma_is_possible_existence"]]
            == 1
        ).any(axis=1),
        "melanoma",
    ] = 1
    ...
    nlp_df.loc[
        (nlp_df["depth_greater_than_breslow_measurement"] == 1)
        & (nlp_df["deep_margin_involvement_negated"] != 1),
        "transection",
    ] = 1
    ...
    nlp_df.loc[
        (nlp_df["ulceration_status"] == "present") & (nlp_df["melanoma"] == 1),
        "ulceration_status",
    ] = 1
    ...
    if binary_classifier:
        ...
        nlp_df.loc[
            ((nlp_df["Mitotic_index"].notna()) & (nlp_df["Mitotic_index"] != 0)),
            "Mitotic_index",
        ] = 1
    ...
    if binary_classifier:
        return (
            nlp_df.groupby("doc_id")
            .agg(
                {
                    "melanoma": "max",
                    "breslow_measurement": "max",
                    "clark": "max",
                    "metastasis": "max",
                    "ulceration_status": "max",
                    "Mitotic_index": "max",
                    "metastasis": "max",
                    "transection": "max",
                }
        ...
        return (
            ...
            .agg(
                {
                    "melanoma": "max",
                    "breslow_measurement": combine_sets,
                    "clark": combine_sets,
                    "metastasis": "max",
                    "ulceration_status": "max",
                    "Mitotic_index": combine_sets,
                    "metastasis": "max",
                    "transection": "max",
                }
            )
