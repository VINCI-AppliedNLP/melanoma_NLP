...
def run_nlp(raw_txt, nlp, n_process=1):
    """
    Processes a list of raw text strings using a spaCy NLP pipeline.
    Args:
        raw_txt (list or iterable): The raw text data to process. Can be a list, pandas Series, or other iterable.
        nlp (spacy.language.Language): The spaCy NLP pipeline to use for processing.
        n_process (int, optional): Number of processes to use for parallel processing. Defaults to 1.
    Returns:
        list: A list of spaCy Doc objects resulting from processing the input text.
    Raises:
        TypeError: If the input cannot be converted to a list.
    """
...
def map_mod(mod, span, ent_label):
    """
    Maps a modifier object, text span, and entity label to a standardized label and value for melanoma pathology NLP extraction.
    Args:
...
def data_transformation(docIDs, docs) -> dict:
    """
...
def flatten_on_relationships(long_df):
    """
    Converts a long-format DataFrame of pathology NLP relationships into a wide-format DataFrame.
    ...
    for col in [
        ...
        if col not in piv_1.columns:
            print(
                "Either no negation, historical, possible existence, or hypothetical contexts detected in this set. Setting negation/history/hypothetical to None"
            )
            piv_1[col] = None
...
def one_hot_encode_hist(piv_1, exclude_nonmelanoma_ulceration=1):
    ...
    piv_1["anchor_mapping"] = piv_1.apply(
        lambda x: (
            ...
            if ((x["is_historical"] == 1) & (x["is_negated"] == 0))
    ...
    piv_1["anchor_mapping"] = piv_1.apply(
        lambda x: (
            ...
            if ((x["is_hypothetical"] == 1) & (x["is_negated"] == 0))
            else x["anchor_mapping"]
        ),
        axis=1,
    )  # is_possible_existence
    piv_1["anchor_mapping"] = piv_1.apply(
        lambda x: (
            ...
            if ((x["is_possible_existence"] == 1) & (x["is_negated"] == 0))
            else x["anchor_mapping"]
        ),
        axis=1,
    )
    ...
    piv_1["metastasis_historical"] = piv_1.apply(
        lambda x: (
            1
            if (
                (x["is_historical"] == 1)
    ...
    piv_1["metastasis_negated"] = piv_1.apply(
        lambda x: 1 if ((x["is_negated"] == 1) & (x["metastasis"] == 1)) else 0, axis=1
    )
    piv_1["metastasis_hypothetical"] = piv_1.apply(
        lambda x: (
            1
            if (
                (x["is_hypothetical"] == 1)
    ...
    piv_1["metastasis_is_possible_existence"] = piv_1.apply(
        lambda x: (
            1
            if (
    ...
    piv_1["metastasis"] = piv_1.apply(
        lambda x: (
            0
            if (
                (x["metastasis_historical"] == 1)
                | (x["metastasis_negated"] == 1)
                | (x["metastasis_hypothetical"] == 1)
                | (x["metastasis_is_possible_existence"] == 1)
            )
            else x["metastasis"]
        ),
        axis=1,
    )  # metastasis should be 0 if it's historical or negated or hypothetical (this is captured separately)
...
def grouping_top(piv_1_encoded, return_max_values=1):
    """
    Groups and aggregates pathology NLP data by topography and related spans, applying custom aggregation logic for melanoma features.
    Args:
    ...
    top_grouped["melanoma_negated"] = (
        top_grouped[[f"{x}_negated" for x in melanoma_dx]].any(axis=1).astype(int)
    )
    top_grouped["melanoma_hypothetical"] = (
        top_grouped[[f"{x}_hypothetical" for x in melanoma_dx]].any(axis=1).astype(int)
    )
    top_grouped["melanoma_is_possible_existence"] = (
        top_grouped[[f"{x}_is_possible_existence" for x in melanoma_dx]]
        .any(axis=1)
        .astype(int)
    )
    top_grouped["melanoma_historical"] = (
        top_grouped[[f"{x}_historical" for x in melanoma_dx]].any(axis=1).astype(int)
    )
